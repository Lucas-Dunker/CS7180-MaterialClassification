...
class MaterialRecognitionSystem:
    ...
    def histogram_intersection_kernel(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
        ...
        return np.array(
            [[np.minimum(x, y).sum() for y in Y] for x in X], 
            dtype=np.float32
        )
    ...
    def predict(self, features: Dict[str, np.ndarray]) -> int:
        ...
        K_test = self.histogram_intersection_kernel(np.array([hist]), self.X_train_stored)
    ...
    def evaluate(self, features_list: List[Dict[str, np.ndarray]], 
