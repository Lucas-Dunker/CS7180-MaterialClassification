"""
The driver code for our material recognition system.
"""
...
from config import FMD_CATEGORIES, TRAIN_PER_CATEGORY, TEST_PER_CATEGORY, MODEL_DIR


def train_material_recognizer(fmd_path: str, 
                              model_name: str = "material_recognition",
                              use_masks: bool = True):
    """
...
def evaluate_model(system: MaterialRecognitionSystem,
                  fmd_path: str,
                  use_masks: bool = True,
    ...
    _, _, _, X_test, y_test, masks_test = (
        split_dataset_per_category(images, labels, masks, TEST_PER_CATEGORY)
    )
    ...
    if save_results:
        ...
        np.save(MODEL_DIR / "true_labels.npy", y_test)
        print("\nPredictions and true labels saved to 'predictions.npy' and 'true_labels.npy'")
    
    return accuracy, predictions


def predict_single_image(system: MaterialRecognitionSystem,
                        image_path: str,
...
def main():
    ...
    parser.add_argument("--fmd_path", type=str, default="./datasets/FMD",
                       help="Path to FMD dataset")
    parser.add_argument("--model_name", type=str, default="material_recognition",
                       help="Name for saved model")
    parser.add_argument("--no_masks", action="store_true",
                       help="Don't use binary masks")
    parser.add_argument("--predict", type=str,
                       help="Path to single image for prediction")
    parser.add_argument("--mask", type=str,
                       help="Path to mask for single image prediction")
