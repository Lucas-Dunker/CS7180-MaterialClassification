...
class GaborFilterBank:
    ...
    def __init__(self, scales: List[float] = FEATURE_CONFIG["gabor_filter"]["scales"], n_orientations: int = FEATURE_CONFIG["gabor_filter"]["n_orientations"]):
...
class FeatureExtractor:
    ...
    def extract_color_features(self, img: np.ndarray, 
                              mask: Optional[np.ndarray] = None,
                              grid_step: int = 5) -> np.ndarray:
        ...
        return (np.array(features, dtype=np.float32) if features 
                else np.array([]).reshape(0, 27).astype(np.float32))
    
    def extract_jet_features(self, img: np.ndarray,
                           mask: Optional[np.ndarray] = None,
                           grid_step: int = 5) -> np.ndarray:
    ...
    def extract_sift_features(self, img: np.ndarray,
                            mask: Optional[np.ndarray] = None,
                            grid_step: int = FEATURE_CONFIG['grid_step']) -> np.ndarray:
        ...
        if keypoints:
            ...
            return (descriptors.astype(np.float32) if descriptors is not None
                   else np.array([]).reshape(0, 128).astype(np.float32))
        else:
            return np.array([]).reshape(0, 128).astype(np.float32)
    
    def extract_curvature_features(self, contours: List[np.ndarray],
                                  scales: List[int] = [],
            ...
            for i in range(0, len(contour), 3):
                ...
                for scale in scales:
                    ...
                    if end - start >= 3:
                        ...
                        if len(segment) >= 3:
                            v1 = (segment[-1][0].astype(np.float32) - 
                                 segment[len(segment) // 2][0].astype(np.float32))
                            v2 = (segment[len(segment) // 2][0].astype(np.float32) - 
        ...
        return (np.array(features, dtype=np.float32) if features
                else np.array([]).reshape(0, len(scales)).astype(np.float32))
    
    def extract_hog_slice(self, magnitude: np.ndarray, 
                         orientation: np.ndarray,
                         cx: int, cy: int, 
                         angle: float,
                         length: int = 18, 
                         width: int = 3) -> Optional[np.ndarray]:
